...
spatial_obj <- Load10X_Spatial(data.dir = spatial_path, filename = "filtered_feature_bc_matrix.h5") ## ZJW edit
spatial_obj$row <- spatial_obj@images$slice1@coordinates$row ## ZJW edit
spatial_obj$col <- spatial_obj@images$slice1@coordinates$col ## ZJW edit
spatial_obj$imagerow <- spatial_obj@images$slice1@coordinates$imagerow ## ZJW edit
spatial_obj$imagecol <- spatial_obj@images$slice1@coordinates$imagecol ## ZJW edit
...
counts <- data.frame(spatial_obj@assays$Spatial@counts) # load in counts matrix  # ZJW edit: RNA->Spatial
...
write.csv(norm_weights, paste0(output_path, '/RCTD_result.csv'), quote = F,row.names = T,col.names = T)
